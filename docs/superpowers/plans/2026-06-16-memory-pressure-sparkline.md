...
- [ ] **無障礙(VoiceOver)**:目前壓力主要靠「標題數字顏色 + 副標文字」傳達,顏色對 VoiceOver 無語義。可在 `MetricCard`/`MemorySection` 加 `accessibilityLabel`(例:`記憶體 79% 壓力警告`),讓壓力不僅靠顏色。`MemorySection.swift` 約 line 20/47。
- [ ] **淺色模式對比**:`PressureColor` 的 `Color.yellow`(警告)在 macOS Light Mode 卡片淺底下對比接近 WCAG AA 邊緣。可改用 `Color(.systemOrange)` 或調整亮度。檔案 `GlanceApp/Dropdown/PressureColor.swift`。
- [ ] **實機目視驗證**:結構已由 build/測試/啟動冒煙驗證,但壓力色的實際綠→黃→紅切換尚未在執行中下拉目視確認。開大型 app 製造記憶體壓力後,確認標題數字、副標、sparkline 三者同步轉色。
- [ ] **(可選)防呆 assert**:`PressureColor.color(forLevel:)` 對非 0/1/2 的輸入靜默回綠;若日後 `memoryPressure` 改存連續值,可在 `default` 加 `assertionFailure`(debug-only)以早期發現誤用。
